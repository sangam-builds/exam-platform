  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        <h1 className="text-2xl font-bold tracking-tight text-white">Accept Invitation</h1>
        <p className="text-sm text-slate-400 mt-1">Set up your teacher account and credentials</p>
      </div>
      ...
      {tokenInfo && (
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="p-4 bg-slate-900/80 border border-slate-800 rounded-xl space-y-2">
            <div className="text-xs text-slate-400">Invited Account:</div>
            <div className="flex items-center justify-between">
              <span className="font-semibold text-white text-sm">{tokenInfo.email}</span>
              <Badge variant="info" size="sm">{tokenInfo.role}</Badge>
            </div>
          </div>
